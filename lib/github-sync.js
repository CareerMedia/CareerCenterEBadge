...
const { PATHS, ensureDir, appendAppErrorLog } = require('./store');
...
function getMutableFiles(options = {}) {
  const includePrefixes = Array.isArray(options.includePrefixes) ? options.includePrefixes.filter(Boolean) : null;
  const staticFiles = [
  ...
  const recursiveDirs = [
    ...
    { localDir: PATHS.docsUploadsDir, repoDir: 'docs/assets/uploads' }
  ];
  ...
  if (!includePrefixes || !includePrefixes.length) {
    return files;
  }
  return files.filter((f) => includePrefixes.some((p) => String(f.repoPath || '').startsWith(p)));
}
...
function resolveRemotePathToLocal(repoPath) {
  const explicit = getMutableFiles().find((f) => f.repoPath === repoPath);
  if (explicit) {
    return explicit.localPath;
  }
  for (const mapping of REMOTE_DIR_TO_LOCAL_DIR) {
...
async function pullRemoteData() {
  ...
  for (const item of allBlobs) {
    const localPath = resolveRemotePathToLocal(item.path);
    if (!localPath) {
      continue;
    }
  ...
  return { ok: true, skipped: false, restored };
}

async function pushLocalData(reason = 'Update badge data', options = {}) {
  const config = getConfig();
  if (!config.enabled) {
    return { ok: false, skipped: true, reason: 'GitHub sync not configured.' };
  }
  ...
  const tree = [];
  for (const file of getMutableFiles(options)) {
...
function queuePushLocalData(reason, options = {}) {
  ...
  syncQueue = syncQueue
    .catch(() => null)
    .then(() => pushLocalData(reason, options))
    .catch((err) => {
    ...
    .finally(() => {
      syncStatus.queuedCount = Math.max(0, syncStatus.queuedCount - 1);
    });
  return syncQueue;
}
...
async function persistMutation(reason, mutateFn, rebuildFn, options = {}) {
  ...
  try {
    ...
    if (syncPush) {
      await queuePushLocalData(reason, options);
    } else {
      queuePushLocalData(reason, options).catch((err) => {
        appendAppErrorLog({
          severity: 'warning',
          source: 'github_sync',
