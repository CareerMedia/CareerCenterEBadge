...
const EMAIL_CONFIG_KEYS = ['emailBrevoEnabled', 'emailBrevoApiKey', 'emailBrevoSenderEmail', 'emailBrevoSenderName', 'emailBrevoReplyTo'];
...
function loadAppState() {
  const fallback = {
    ...
    bulkIssueJobs: loadBulkIssueJobs()
  };
  ...
  return {
    ...
    bulkIssueJobs: Array.isArray(stored.bulkIssueJobs) ? stored.bulkIssueJobs : fallback.bulkIssueJobs,
    analyticsSummary: {
      ...DEFAULT_ANALYTICS_SUMMARY,
...
function hydrateFilesFromAppState() {
  ...
  writeJson(PATHS.bulkIssueJobsFile, Array.isArray(state.bulkIssueJobs) ? state.bulkIssueJobs : []);
  writeJson(PATHS.analyticsSummaryFile, {
    ...DEFAULT_ANALYTICS_SUMMARY,
