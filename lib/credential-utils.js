...
function normalizeVerificationSections(record = {}) {
  const source = record && typeof record.verificationSections === 'object' && record.verificationSections ? record.verificationSections : record || {};
  return {
    recipient: source.recipient !== false,
    meaning: source.meaning !== false,
    criteria: source.criteria !== false,
    issuerTrust: source.issuerTrust !== false,
    evidence: source.evidence !== false,
    skills: source.skills !== false,
    pathway: source.pathway !== false,
    certificate: source.certificate !== false
  };
}
...
function absolutizeUrl(value, ...bases) {
  ...
  for (const base of bases) {
    ...
    try {
      if (/^https?:\/\//i.test(candidate)) {
        return new URL(raw.replace(/^\/+/, ''), candidate.replace(/\/+$/, '') + '/').href;
      }
    } catch (error) {
    }
  }
...
function buildEmailSignatureHtml(badge, siteConfig = {}) {
  const publicUrl = absolutizeUrl(badge.publicUrl, siteConfig.publicSiteUrl);
  ...
  const safeImage = absolutizeUrl(
    badge.badgeImage,
    publicUrl,
    siteConfig.publicSiteUrl,
    'https://csun.edu/'
  ) || absolutizeUrl('assets/badges/career-champion-badge.svg', publicUrl, siteConfig.publicSiteUrl, 'https://csun.edu/');
  return [
    ...
    '        <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;border-spacing:0;background:#ffffff;border:1px solid #e7dfde;border-radius:999px;">',
    '          <tr>',
    `            <td style="padding:8px 10px 8px 12px;vertical-align:middle;"><img src="${safeImage}" width="38" height="38" alt="${badgeTitle}" style="display:block;width:38px;height:38px;border:0;outline:none;text-decoration:none;" /></td>`,
    '            <td style="padding:8px 14px 8px 0;vertical-align:middle;font-family:Arial,Helvetica,sans-serif;white-space:nowrap;">',
    '              <span style="display:block;color:#8a1a25;font-size:11px;line-height:1.15;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;">Certified</span>',
    `              <span style="display:block;color:#111827;font-size:15px;line-height:1.2;font-weight:700;">${badgeTitle}</span>`,
    `              <span style="display:block;color:#6b7280;font-size:11px;line-height:1.2;">Verified by ${issuerName}</span>`,
    '            </td>',
    '          </tr>',
    '        </table>',
    '      </a>',
...
function buildEmailSignaturePlainText(badge, siteConfig = {}) {
  const badgeTitle = String(badge.badgeTitle || 'Credential').trim() || 'Credential';
  const issuerName = String(badge.issuerName || siteConfig.organizationName || 'CSUN Career Center').trim();
  const publicUrl = absolutizeUrl(badge.publicUrl, siteConfig.publicSiteUrl);
  return `Certified ${badgeTitle} | Verified by ${issuerName} | ${publicUrl}`;
}
