...
function buildEmailSignatureHtml(badge, siteConfig = {}) {
  const publicUrl = escapeAttribute(String(badge.publicUrl || '').trim());
  const badgeTitle = escapeHtml(String(badge.badgeTitle || 'Credential').trim() || 'Credential');
  const imageUrl = String(badge.badgeImage || '').trim();
  const issuerName = escapeHtml(String(badge.issuerName || siteConfig.organizationName || 'CSUN Career Center').trim());
  const safeImage = escapeAttribute(imageUrl || (String(siteConfig.publicSiteUrl || '').replace(/\/+$/, '') + '/assets/badges/career-champion-badge.svg'));
  return [
    '<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;border-spacing:0;">',
    '  <tr>',
    '    <td style="padding:0;border:1px solid #e7dfde;border-radius:999px;background:#ffffff;">',
    '      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;border-spacing:0;">',
    '        <tr>',
    `          <td style="padding:8px 10px 8px 12px;vertical-align:middle;"><a href="${publicUrl}" target="_blank" style="text-decoration:none;display:block;"><img src="${safeImage}" width="38" height="38" alt="${badgeTitle}" style="display:block;width:38px;height:38px;border:0;outline:none;text-decoration:none;" /></a></td>`,
    '          <td style="padding:8px 14px 8px 0;vertical-align:middle;font-family:Arial,Helvetica,sans-serif;white-space:nowrap;">',
    `            <a href="${publicUrl}" target="_blank" style="text-decoration:none;display:block;">`,
