...
const { AWARD_EMAIL_TEMPLATE_TAGS } = require('./brevo-mailer');
const { getBuiltinAwardEmailParts } = require('./award-email-shared');
...
function formatEmailLogDetailsCell(row) {
  const status = String(row.status || '');
  if (status === 'error') {
    const bits = [];
    bits.push(`<strong>${escapeHtml(row.reason || 'error')}</strong>`);
    if (row.errorMessage) {
      bits.push(
        `<div class="table-subtext email-log-detail">${escapeHtml(String(row.errorMessage))}</div>`
      );
    }
    const meta = [];
    const http = row.httpStatus;
    if (http !== undefined && http !== null && String(http) !== '' && Number(http) > 0) {
      meta.push(`HTTP ${http}`);
    } else if (Number(http) === 0) {
      meta.push('No HTTP response');
    }
    if (row.apiKeySource) {
      let src =
        row.apiKeySource === 'env'
          ? 'REST key from host environment'
          : row.apiKeySource === 'stored'
            ? 'REST key from saved Email settings'
            : 'No REST API key resolved';
      if (row.apiKeySource === 'env' && row.brevoHostEnvVar) {
        src = `REST key from host env (${row.brevoHostEnvVar})`;
      }
      meta.push(src);
    }
    if (row.brevoReachable === false) {
      meta.push('Brevo not contacted');
    }
    if (meta.length) {
      bits.push(`<div class="table-subtext email-log-meta">${escapeHtml(meta.join(' · '))}</div>`);
    }
    if (row.brevoResponseBody) {
      bits.push(
        `<div class="table-subtext email-log-detail">${escapeHtml(String(row.brevoResponseBody))}</div>`
      );
    }
    return bits.join('');
  }
  if (status === 'skipped') {
    const parts = [`<span>${escapeHtml(row.reason || 'skipped')}</span>`];
    if (row.transport === 'api' && row.reason === 'no_api_key') {
      parts.push(
        `<div class="table-subtext">Add <code>BREVO_API_KEY</code> on the host (e.g. Render) or save a key on the Email tab. Sending can run without enabling the checkbox when the env key and sender email are set.</div>`
      );
    }
    return parts.join('');
  }
  if (row.errorMessage) {
    return escapeHtml(row.errorMessage);
  }
  return escapeHtml(row.reason || '');
}
...
function adminLayout({ title, active = 'dashboard', body, notice = '', extraHead = '', extraScripts = '' }) {
  const nav = [
    ...
    { key: 'bulk-issue', href: '/admin/bulk-issue', label: 'Bulk issue' },
    { key: 'templates', href: '/admin/templates', label: 'Badge templates' },
    { key: 'settings', href: '/admin/settings', label: 'Settings' },
    { key: 'email', href: '/admin/email', label: 'Email' },
    { key: 'email-log', href: '/admin/email-log', label: 'Email log' },
    { key: 'debug-log', href: '/admin/debug-log', label: 'Debug log' },
    { key: 'jobs', href: '/admin/jobs', label: 'Background jobs' },
    ...
    ? badges
        ...
        <tr>
          <td>
            ...
            <div class="table-subtext">${escapeHtml(badge.awardeeEmail || '')}</div>
          </td>
    ...
    <section class="admin-panel">
      <form method="post" action="/admin/issue" class="admin-form admin-form--stacked">
        <div class="form-grid form-grid--two">
          ...
          <label><span>Recipient email</span><input type="email" name="awardeeEmail" required /></label>
...
function renderTemplatesPage({ templates, editTemplate = null, notice = '', siteConfig, certificateTemplate }) {
  ...
  const current = editTemplate || {
    ...
    verificationSections: { recipient: true, meaning: true, criteria: true, issuerTrust: true, evidence: true, skills: true, pathway: true, certificate: true },
  ...
  const awardEmailList = Array.isArray(siteConfig.emailAwardTemplates) ? siteConfig.emailAwardTemplates : [];
  const awardEmailOptions = [
    '<option value="">Site default (fallback template)</option>',
    ...awardEmailList.map(
      (t) =>
        `<option value="${escapeAttribute(t.id)}" ${String(current.emailAwardTemplateId || '') === t.id ? 'selected' : ''}>${escapeHtml(
          t.name || t.id
        )}</option>`
    )
  ].join('');
  const effectiveTemplateCertificate = getCertificateTemplateForTemplate(current, certificateTemplate);
  const certificatePreviewBackground = current.certificateBackground || effectiveTemplateCertificate.backgroundImage || certificateTemplate.backgroundImage;
  const certificatePreviewUrl = resolveAdminAssetUrl(certificatePreviewBackground);
      ...
      <form method="post" action="/admin/templates/save" class="admin-form admin-form--stacked" id="badgeTemplateForm">
        <div class="form-grid form-grid--two">
          ...
          <label class="form-span-2">
            <span>Award email template</span>
            <select name="emailAwardTemplateId">${awardEmailOptions}</select>
            <span class="muted email-template-assignment-note">When this badge type is issued, the selected message is sent (configure templates under <strong>Email</strong>). Leave on “Site default” to use the fallback template.</span>
          </label>
        ...
        <section class="template-config-shell">
          <div class="template-config-shell__header">
            <div>
              <p class="admin-section-label">Verification page cards</p>
              <h3>Choose which sections show on the public badge page</h3>
            </div>
          </div>
          <div class="section-toggle-grid">
            <label class="toggle-check"><input type="checkbox" name="sectionRecipient" ${current.verificationSections && current.verificationSections.recipient !== false ? 'checked' : ''} /><span>Recipient card</span></label>
            <label class="toggle-check"><input type="checkbox" name="sectionMeaning" ${current.verificationSections && current.verificationSections.meaning !== false ? 'checked' : ''} /><span>Meaning card</span></label>
            <label class="toggle-check"><input type="checkbox" name="sectionCriteria" ${current.verificationSections && current.verificationSections.criteria !== false ? 'checked' : ''} /><span>Criteria section inside Meaning</span></label>
            <label class="toggle-check"><input type="checkbox" name="sectionIssuerTrust" ${current.verificationSections && current.verificationSections.issuerTrust !== false ? 'checked' : ''} /><span>Issuer trust card</span></label>
            <label class="toggle-check"><input type="checkbox" name="sectionEvidence" ${current.verificationSections && current.verificationSections.evidence !== false ? 'checked' : ''} /><span>Evidence card</span></label>
            <label class="toggle-check"><input type="checkbox" name="sectionSkills" ${current.verificationSections && current.verificationSections.skills !== false ? 'checked' : ''} /><span>Skills and standards card</span></label>
            <label class="toggle-check"><input type="checkbox" name="sectionPathway" ${current.verificationSections && current.verificationSections.pathway !== false ? 'checked' : ''} /><span>Pathway card</span></label>
            <label class="toggle-check"><input type="checkbox" name="sectionCertificate" ${current.verificationSections && current.verificationSections.certificate !== false ? 'checked' : ''} /><span>Certificate download card</span></label>
          </div>
        </section>
        ...
        <section class="template-config-shell">
          <div class="template-config-shell__header">
            ...
            <div class="inline-actions">
              ...
              <button type="button" class="button button--ghost" id="certificateAutoCenterBtn">Auto-center name &amp; date</button>
            </div>
          </div>
          <div class="coordinate-editor-grid">
            <div class="certificate-coordinate-preview" id="certificateCoordinatePreview">
              <img id="certificateCoordinateImage" src="${escapeAttribute(certificatePreviewUrl)}" alt="Certificate preview" data-background-path="${escapeAttribute(certificatePreviewBackground)}" />
            ...
            <div class="form-grid form-grid--two" id="certificateCoordinateFields">
              <input type="hidden" name="certificateBackgroundOverride" id="certificateBackgroundOverride" value="${escapeAttribute(certificatePreviewBackground)}" />
  ...
  return adminLayout({
    ...
    body,
    notice,
    extraScripts: `<script>window.__TEMPLATES__ = ${serializeForScript(templates)}; window.__TEMPLATE_EDITOR__ = ${serializeForScript({ backgroundImage: certificatePreviewBackground, nameX: effectiveTemplateCertificate.name.x, nameY: effectiveTemplateCertificate.name.y, dateX: effectiveTemplateCertificate.date.x, dateY: effectiveTemplateCertificate.date.y })};</script>`
  });
}
...
function renderEmailPage({
  siteConfig,
  notice = '',
  envBrevoKeyConfigured = false,
  brevoEnv = {},
  envSmtpConfigured = false
}) {
  const enabled = Boolean(siteConfig.emailBrevoEnabled);
  const hasStoredKey = Boolean(String(siteConfig.emailBrevoApiKey || '').trim());
  const transport = String(siteConfig.emailBrevoTransport || 'api').toLowerCase() === 'smtp' ? 'smtp' : 'api';
  const senderEmail = escapeAttribute(siteConfig.emailBrevoSenderEmail || '');
  const senderName = escapeAttribute(siteConfig.emailBrevoSenderName || 'CSUN Career Center');
  const replyTo = escapeAttribute(siteConfig.emailBrevoReplyTo || '');
  const awardTemplates = Array.isArray(siteConfig.emailAwardTemplates) ? siteConfig.emailAwardTemplates : [];
  const defaultTplId = String(siteConfig.emailAwardDefaultTemplateId || (awardTemplates[0] && awardTemplates[0].id) || 'default');
  const defaultOptions = awardTemplates
    .map(
      (t) =>
        `<option value="${escapeAttribute(t.id)}" ${t.id === defaultTplId ? 'selected' : ''}>${escapeHtml(
          t.name || t.id
        )}</option>`
    )
    .join('');
  const tagRows = AWARD_EMAIL_TEMPLATE_TAGS.map(
    (row) =>
      `<tr><td><code>${escapeHtml(row.token)}</code></td><td>${escapeHtml(row.description)}</td></tr>`
  ).join('');
  const be = brevoEnv || {};
  let envDiagHtml = '';
  if (be.brevoVarPresent && !be.hostKeyActive) {
    envDiagHtml =
      '<p class="admin-error"><strong><code>BREVO_API_KEY</code> is set on this host but the value is empty after cleanup</strong> (quotes, line breaks, or invisible characters only). Paste a fresh <strong>v3 REST API key</strong> from Brevo → SMTP &amp; API → API keys—not the SMTP password.</p>';
  } else if (be.hostKeyActive && be.hostKeyVarName === 'SENDINBLUE_API_KEY') {
    envDiagHtml = `<p class="muted">REST key is loaded from legacy host variable <code>SENDINBLUE_API_KEY</code> (length ${be.hostKeyLength}). Prefer renaming to <code>BREVO_API_KEY</code> on Render for clarity.</p>`;
  } else if (be.hostKeyActive) {
    envDiagHtml = `<p class="muted">REST API key is read from host environment variable <code>${escapeHtml(
      be.hostKeyVarName || 'BREVO_API_KEY'
    )}</code> (normalized length <strong>${escapeHtml(
      String(be.hostKeyLength || 0)
    )}</strong>). That value is sent to Brevo as the <code>api-key</code> header. If Brevo returns <em>401 Key not found</em>, the secret on Render does not match a valid API key—regenerate it in Brevo and update the environment variable, then redeploy.</p>`;
  } else {
    envDiagHtml =
      '<p class="muted">No usable REST key in host environment. Set <code>BREVO_API_KEY</code> (or legacy <code>SENDINBLUE_API_KEY</code>) on Render for the <strong>web</strong> service, or paste a key below. Saved keys in this form are used only when host env keys are unset.</p>';
  }

  const body = `
    <section class="admin-header">
      <div>
        <p class="admin-section-label">Email</p>
        <h2>Brevo transactional email</h2>
        <p>When enabled, each issued badge triggers an email to the awardee’s address using Brevo’s <code>POST /v3/smtp/email</code> API (same flow for single issue, public generator, and bulk issue).</p>
      </div>
    </section>

    <section class="admin-panel">
      <form method="post" action="/admin/email/save" class="admin-form admin-form--stacked" id="emailAwardSettingsForm">
        <label class="toggle-check">
          <input type="checkbox" name="emailBrevoEnabled" value="true" ${enabled ? 'checked' : ''} />
          <span>Send award email automatically when a badge is issued</span>
        </label>

        <div class="admin-section-heading">
          <div>
            <p class="admin-section-label">Brevo credentials</p>
            <h3>API key and sender</h3>
          </div>
        </div>
        <p class="muted">Brevo settings are saved to <code>data/email-config.json</code> (separate from general site settings). With GitHub persistence enabled, that file is stored on your data branch like other badge records, so app deploys and code updates do not wipe your email configuration.</p>
        <p class="muted"><strong>Sending can run without checking the box above</strong> if a REST key is available from the host (<code>BREVO_API_KEY</code> first, then legacy <code>SENDINBLUE_API_KEY</code>) <em>and</em> a verified sender email is saved here. Otherwise, turn on the checkbox and supply credentials for the transport you choose.</p>
        <p class="muted">In Brevo: <strong>SMTP &amp; API</strong> → <strong>API keys</strong> (REST v3 transactional key). The SMTP tab password is <strong>not</strong> the REST key. Add and verify your sender under <strong>Senders</strong>.</p>

        ${envDiagHtml}

        <label>
          <span>Send via</span>
          <select name="emailBrevoTransport">
            <option value="api" ${transport === 'api' ? 'selected' : ''}>Brevo REST API (recommended)</option>
            <option value="smtp" ${transport === 'smtp' ? 'selected' : ''}>Brevo SMTP relay</option>
          </select>
        </label>
        <p class="muted">SMTP uses server environment variables only (not saved in Git): <code>BREVO_SMTP_LOGIN</code>, <code>BREVO_SMTP_PASSWORD</code> (SMTP key from Brevo), optional <code>BREVO_SMTP_HOST</code> (default <code>smtp-relay.brevo.com</code>), optional <code>BREVO_SMTP_PORT</code> (default <code>587</code>, or <code>465</code> with SSL).</p>
        ${
          envSmtpConfigured
            ? '<p class="muted">Detected <code>BREVO_SMTP_LOGIN</code> and <code>BREVO_SMTP_PASSWORD</code> on this server.</p>'
            : ''
        }

        <label>
          <span>Brevo API key (saved in settings)</span>
          <input type="password" name="emailBrevoApiKey" autocomplete="new-password" placeholder="${hasStoredKey ? 'Leave blank to keep the saved key' : 'Paste your Brevo API key'}" />
        </label>
        ${hasStoredKey ? '<p class="muted">A key is already stored in site settings. Leave the field blank to keep it, or paste a new key to replace it.</p>' : ''}
        ${
          envBrevoKeyConfigured
            ? '<p class="muted">A REST key from the host environment is active and <strong>overrides</strong> the saved key above when sending.</p>'
            : ''
        }

        <label class="toggle-check">
          <input type="checkbox" name="emailBrevoClearApiKey" value="true" />
          <span>Remove saved API key from these settings (optional; use only if you rely on <code>BREVO_API_KEY</code> / <code>SENDINBLUE_API_KEY</code> on the server)</span>
        </label>

        <div class="form-grid form-grid--two">
          <label>
            <span>Sender email (Brevo-verified)</span>
            <input type="email" name="emailBrevoSenderEmail" value="${senderEmail}" required />
          </label>
          <label>
            <span>Sender display name</span>
            <input type="text" name="emailBrevoSenderName" value="${senderName}" required />
          </label>
          <label class="form-span-2">
            <span>Reply-to email (optional)</span>
            <input type="email" name="emailBrevoReplyTo" value="${replyTo}" placeholder="career.center@csun.edu" />
          </label>
        </div>

        <p class="muted">Set <strong>Public site URL</strong> on the Settings tab so <code>{badge-link}</code> uses a full <code>https://</code> URL when possible.</p>

        <input type="hidden" name="emailAwardTemplatesJson" id="emailAwardTemplatesJson" value="${escapeAttribute(
          JSON.stringify(awardTemplates)
        )}" />
        <input type="hidden" name="saveMode" id="saveModeInput" value="full" />

        <div class="admin-section-heading">
          <div>
            <p class="admin-section-label">Award email copy</p>
            <h3>Templates &amp; merge tags</h3>
          </div>
        </div>
        <p class="muted">Create multiple templates here, then assign one to each <strong>badge template</strong> on <strong>Badge templates → Edit</strong> (or leave unassigned to use the fallback below). Use merge tags in curly braces (case-insensitive). In <strong>Plain text</strong>, add links as <code>[visible text](https://example.com)</code>; switching tabs syncs with HTML.</p>
        <div class="table-wrap email-template-tags-wrap">
          <table class="data-table">
            <thead><tr><th>Tag</th><th>Inserts</th></tr></thead>
            <tbody>${tagRows}</tbody>
          </table>
        </div>

        <label>
          <span>Fallback template (when a badge template has no assignment)</span>
          <select name="emailAwardDefaultTemplateId" id="emailAwardDefaultTemplateId">${defaultOptions}</select>
        </label>

        <div class="email-template-editor-shell">
          <div class="inline-actions email-template-toolbar">
            <label class="email-template-toolbar__picker">
              <span>Editing template</span>
              <select id="emailTemplatePicker"></select>
            </label>
            <button type="button" class="button button--ghost" id="emailTemplateAdd">Add template</button>
            <button type="button" class="button button--small button--danger" id="emailTemplateDelete">Delete template</button>
          </div>
          <label>
            <span>Template name</span>
            <input type="text" id="emailTplName" maxlength="120" autocomplete="off" />
          </label>
          <label>
            <span>Template ID (set when you add a template)</span>
            <input type="text" id="emailTplId" maxlength="80" readonly autocomplete="off" class="muted" />
          </label>
          <label>
            <span>Subject line</span>
            <input type="text" id="emailTplSubject" maxlength="998" autocomplete="off" />
          </label>
          <div class="wp-like-tabs">
            <div class="wp-like-tabs__bar" role="tablist">
              <button type="button" class="button button--small wp-like-tabs__btn is-active" data-tab="plain" role="tab" aria-selected="true">Plain text</button>
              <button type="button" class="button button--small wp-like-tabs__btn" data-tab="html" role="tab" aria-selected="false">HTML</button>
            </div>
            <div class="wp-like-tabs__panel" id="emailTplPanePlain" role="tabpanel">
              <p class="muted">Blank line between paragraphs. Links: <code>[Career Center](https://csun.edu/career)</code> (http or https only).</p>
              <textarea id="emailTplBodyPlain" rows="16" class="email-template-textarea" spellcheck="true"></textarea>
            </div>
            <div class="wp-like-tabs__panel" id="emailTplPaneHtml" role="tabpanel" hidden>
              <p class="muted">Full HTML email document or a fragment (fragments are wrapped on send). Switch to <strong>Plain text</strong> to auto-generate a text version from this HTML.</p>
              <textarea id="emailTplBodyHtml" rows="16" class="email-template-textarea" spellcheck="false"></textarea>
            </div>
          </div>
        </div>

        <div class="inline-actions">
          <button type="button" class="button button--ghost" id="saveTemplatesOnlyBtn" data-save-mode="templates_only">Save template changes</button>
          <button type="button" id="saveAllEmailBtn" data-save-mode="full">Save all email settings</button>
        </div>
        <p class="muted"><strong>Save template changes</strong> updates template content and assignment fallback only. <strong>Save all email settings</strong> also validates Brevo REST key and sender configuration.</p>
        <p class="muted">When using <strong>Save all email settings</strong> with REST API, the app checks your key with Brevo (<code>GET /v3/account</code>). If that fails, nothing is saved—fix the key on Render or here, then try again.</p>
      </form>
    </section>
  `;

  const awardState = serializeForScript({
    templates: awardTemplates,
    defaultId: defaultTplId
  });
  const builtinAward = serializeForScript(getBuiltinAwardEmailParts());
  return adminLayout({
    title: 'Email',
    active: 'email',
    body,
    notice,
    extraHead: `<script>window.__EMAIL_BUILTIN__ = ${builtinAward}; window.__EMAIL_AWARD_STATE__ = ${awardState};</script>`
  });
}

function renderEmailLogPage({ entries = [], notice = '' }) {
  const rows = entries.length
    ? entries
        ...
        <tr>
          ...
          <td>${escapeHtml(row.badgeTitle || '')}<div class="table-subtext">${escapeHtml(row.badgeId || '')}</div></td>
          <td><span class="status-tag">${escapeHtml(row.status || '')}</span></td>
          <td>${escapeHtml(row.transport || '')}</td>
          <td>${escapeHtml(row.messageId || '')}</td>
          <td class="email-log-details">${formatEmailLogDetailsCell(row)}</td>
        </tr>`
        )
        .join('')
    : '<tr><td colspan="7" class="table-empty">No email attempts logged yet. Issue a badge to see entries here (including skipped attempts and errors).</td></tr>';
  const body = `
    <section class="admin-header">
      <div>
        <p class="admin-section-label">Email log</p>
        <h2>Outbound award emails</h2>
        <p>Each row is one attempt when a badge was issued (single issue, public generator, or bulk). Skipped rows explain why nothing was sent (for example disabled, missing API key, or invalid recipient). Errors list the server message, HTTP status when Brevo responded, and whether the REST key came from host <code>BREVO_API_KEY</code> or saved settings—useful when debugging Render.</p>
      </div>
    </section>
    <section class="admin-panel">
      <div class="table-wrap">
  ...
  `;
  return adminLayout({ title: 'Email log', active: 'email-log', body, notice });
}

function renderDebugLogPage({ entries = [], notice = '' }) {
  const rows = entries.length
    ? entries
        .map((row) => {
          const stackPreview = String(row.stack || '')
            .split('\n')
            .slice(0, 6)
            .join('\n');
          const pathLine = [escapeHtml(row.method || ''), escapeHtml(row.path || '')].filter(Boolean).join(' ');
          return `
        <tr>
          <td>${escapeHtml(row.timestamp || '')}</td>
          <td><span class="status-tag status-tag--${escapeAttribute(String(row.severity || 'error').replace(/[^a-z-]/gi, ''))}">${escapeHtml(row.severity || '')}</span></td>
          <td>${escapeHtml(row.source || '')}</td>
          <td>${pathLine || '—'}</td>
          <td>${escapeHtml(row.message || '')}</td>
          <td>${escapeHtml(row.context || '')}</td>
          <td><pre class="error-stack-preview">${escapeHtml(stackPreview || '—')}</pre></td>
        </tr>`;
        })
        .join('')
    : '<tr><td colspan="7" class="table-empty">No server-side errors recorded yet. Entries appear when requests throw, persistence to GitHub fails, startup sync fails, or an unhandled promise rejection occurs.</td></tr>';
  const body = `
    <section class="admin-header">
      <div>
        <p class="admin-section-label">Debug log</p>
        <h2>Server error diagnostics</h2>
        <p>Read-only log of exceptions and failed operations on this Node host (not browser console). Use with the email log to trace delivery issues. Newest entries first; capped for display.</p>
      </div>
    </section>
    <section class="admin-panel">
      <div class="table-wrap">
  ...
  `;
  return adminLayout({ title: 'Debug log', active: 'debug-log', body, notice });
}
...
function renderBulkIssuePage({ templates = [], jobs = [], notice = '', defaultDate = '' }) {
  const templateOptions = templates
    .map((template) => `<option value="${escapeAttribute(template.id)}">${escapeHtml(template.title)}</option>`)
    .join('');
  const jobRows = jobs.length
    ...
    `).join('')
    : '<tr><td colspan="6" class="empty-row">No bulk issue jobs yet.</td></tr>';
  const body = `
    <section class="admin-header">
      <div>
        <p class="admin-section-label">Bulk issue badges</p>
        <h2>Issue many credentials from one CSV</h2>
        <p>Upload a CSV with recipient name and email, validate rows first, then issue everything in one run.</p>
      </div>
      <div class="inline-actions">
        <a class="button button--ghost" href="/admin/bulk-issue/template.csv">Download CSV template</a>
      </div>
    </section>

    <section class="admin-panel">
      <form method="post" action="/admin/bulk-issue/validate" class="admin-form admin-form--stacked" id="bulkIssueForm">
        <div class="form-grid form-grid--two">
          <label>
            <span>Badge template for all rows</span>
          ...
          <label>
            <span>CSV file</span>
            <input type="file" data-load-file="bulkIssueCsv" accept=".csv,text/csv" required />
          </label>
          <label>
            <span>Issue date source</span>
            <select name="issueDateMode" required>
              <option value="today" selected>Use today for all rows (default)</option>
              <option value="report">Use issue_date from uploaded report</option>
            </select>
          </label>
          <label>
            <span>Default issue date (today)</span>
            <input type="text" value="${escapeAttribute(defaultDate)}" readonly />
          </label>
          <label class="form-span-2">
            <span>CSV content</span>
            <textarea id="bulkIssueCsv" name="csvContent" rows="10" placeholder="awardee_name,awardee_email,issue_date&#10;Jane Doe,jane@csun.edu,2026-04-28" required></textarea>
          </label>
        </div>
        <div class="inline-actions">
          <button type="submit">Validate CSV</button>
          <span class="muted">Required columns: <code>awardee_name</code>, <code>awardee_email</code>. Optional: <code>issue_date</code>.</span>
        </div>
      </form>
    </section>
    ...
    <section class="admin-panel">
      <div class="admin-section-heading">
        <div>
          <p class="admin-section-label">Job history</p>
          <h3>Previous bulk issue jobs</h3>
        </div>
      </div>
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody id="bulkIssueJobsBody">${jobRows}</tbody>
        </table>
      </div>
    </section>
  `;
  return adminLayout({
    ...
    body,
    notice,
    extraScripts: `<script>window.__BULK_ISSUE_JOBS__ = ${serializeForScript(jobs)};</script>`
  });
}

function renderBulkIssueValidationPage({ job, notice = '' }) {
  const rows = Array.isArray(job.rows) ? job.rows : [];
  const hasErrors = rows.some((row) => Array.isArray(row.errors) && row.errors.length);
  const rowMarkup = rows.map((row, index) => `
    <tr>
      <td>${escapeHtml(String(row.rowNumber || index + 2))}</td>
      <td><input type="text" name="row_${index}_awardeeName" value="${escapeAttribute(row.awardeeName || '')}" /></td>
      <td><input type="email" name="row_${index}_awardeeEmail" value="${escapeAttribute(row.awardeeEmail || '')}" /></td>
      <td><input type="text" name="row_${index}_issueDate" value="${escapeAttribute(row.issueDate || '')}" ${job.issueDateMode === 'today' ? 'readonly' : ''} /></td>
      <td>${Array.isArray(row.errors) && row.errors.length ? row.errors.map((error) => `<div class="table-subtext">${escapeHtml(error)}</div>`).join('') : '<span class="status-tag">valid</span>'}</td>
    </tr>
  `).join('');
  const body = `
    <section class="admin-header">
      <div>
        <p class="admin-section-label">Bulk issue validation</p>
        <h2>Review and fix CSV rows</h2>
        <p>Correct any row-level errors below. Valid rows stay in place so you only fix what is needed.</p>
      </div>
      <div class="inline-actions">
        <a class="button button--ghost" href="/admin/bulk-issue">Back to upload</a>
      </div>
    </section>
    <section class="admin-panel">
      <form method="post" action="/admin/bulk-issue/validate" class="admin-form admin-form--stacked">
        <input type="hidden" name="jobId" value="${escapeAttribute(job.id)}" />
        <input type="hidden" name="badgeTemplateId" value="${escapeAttribute(job.badgeTemplateId)}" />
        <input type="hidden" name="issueDateMode" value="${escapeAttribute(job.issueDateMode || 'today')}" />
        <input type="hidden" name="rowCount" value="${escapeAttribute(String(rows.length))}" />
        <div class="table-wrap">
          <table class="data-table">
            <thead>
              <tr><th>Row</th><th>Recipient name</th><th>Recipient email</th><th>Issue date</th><th>Validation</th></tr>
            </thead>
            <tbody>${rowMarkup || '<tr><td colspan="5" class="empty-row">No rows found.</td></tr>'}</tbody>
          </table>
        </div>
        <div class="inline-actions">
          <button type="submit" class="button button--ghost">Re-validate rows</button>
        </div>
      </form>
    </section>
    <section class="admin-panel">
      <form method="post" action="/admin/bulk-issue/start" class="admin-form admin-form--stacked">
        <input type="hidden" name="jobId" value="${escapeAttribute(job.id)}" />
        <div class="inline-actions">
          <button type="submit" ${hasErrors ? 'disabled' : ''}>Issue badges now</button>
          <span class="muted">${hasErrors ? 'Resolve all blocking errors before issuing.' : 'All rows are valid and ready to issue.'}</span>
        </div>
      </form>
    </section>
  `;
  return adminLayout({ title: 'Bulk issue validation', active: 'bulk-issue', body, notice });
}

function renderBulkIssueSuccessPage({ job, notice = '' }) {
  const rows = Array.isArray(job.results) ? job.results : [];
  const resultRows = rows.length
    ? rows.map((entry) => `
      <tr>
        <td>${escapeHtml(entry.awardeeName || '')}</td>
        <td>${escapeHtml(entry.badgeId || '')}</td>
        <td>
          <div class="inline-actions">
            <a class="text-link" href="${escapeAttribute(entry.publicUrl || '#')}" target="_blank" rel="noreferrer">Open</a>
            <button type="button" class="button button--small button--ghost" data-copy="${escapeAttribute(entry.publicUrl || '')}">Copy link</button>
          </div>
        </td>
      </tr>
    `).join('')
    : '<tr><td colspan="3" class="empty-row">No badges were created in this run.</td></tr>';
  const body = `
    <section class="admin-header">
      <div>
        <p class="admin-section-label">Bulk issue complete</p>
        <h2>Bulk run ${escapeHtml(job.id || '')} finished</h2>
        <p>Use these verification links to open or copy each generated badge page.</p>
      </div>
      <div class="inline-actions">
        <a class="button button--ghost" href="/admin/bulk-issue">Start another bulk run</a>
      </div>
    </section>
    <section class="stats-grid">
      <article class="stat-card"><span>Total rows</span><strong>${escapeHtml(String(job.totalRows || 0))}</strong></article>
      <article class="stat-card"><span>Created</span><strong>${escapeHtml(String(job.completedRows || 0))}</strong></article>
      <article class="stat-card"><span>Failed</span><strong>${escapeHtml(String(job.failedRows || 0))}</strong></article>
    </section>
    <section class="admin-panel">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>Recipient</th><th>Credential ID</th><th>Verification page</th></tr></thead>
          <tbody>${resultRows}</tbody>
        </table>
      </div>
    </section>
  `;
  return adminLayout({ title: 'Bulk issue success', active: 'bulk-issue', body, notice });
}

function renderBulkIssueProgressPage({ job, notice = '' }) {
  const jobId = escapeAttribute(job && job.id ? job.id : '');
  const status = escapeHtml(String((job && job.status) || 'pending'));
  const total = escapeHtml(String((job && job.totalRows) || 0));
  const completed = escapeHtml(String((job && job.completedRows) || 0));
  const failed = escapeHtml(String((job && job.failedRows) || 0));
  const percentRaw =
    job && typeof job.progressPercent === 'number' && Number.isFinite(job.progressPercent) ? job.progressPercent : 0;
  const percent = Math.max(0, Math.min(100, Math.round(percentRaw)));
  const body = `
    <section class="admin-header">
      <div>
        <p class="admin-section-label">Bulk issue in progress</p>
        <h2>Issuing badges…</h2>
        <p>Keep this tab open. Progress updates automatically and will redirect when complete.</p>
      </div>
      <div class="inline-actions">
        <a class="button button--ghost" href="/admin/bulk-issue">Back to bulk issue</a>
      </div>
    </section>

    <section class="admin-panel">
      <div class="bulk-progress-shell" data-job-id="${jobId}">
        <div class="bulk-progress-meta">
          <div><strong>Job</strong> <code>${escapeHtml(jobId)}</code></div>
          <div class="muted">Status: <span id="bulkProgressStatus">${status}</span></div>
        </div>
        <div class="bulk-progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${escapeAttribute(
          String(percent)
        )}">
          <div class="bulk-progress-bar__fill" id="bulkProgressFill" style="width:${escapeAttribute(
            String(percent)
          )}%"></div>
        </div>
        <div class="bulk-progress-stats">
          <span><strong id="bulkProgressPercent">${escapeHtml(String(percent))}</strong>%</span>
          <span class="muted"><span id="bulkProgressCompleted">${completed}</span> / <span id="bulkProgressTotal">${total}</span> created</span>
          <span class="muted">Failed: <span id="bulkProgressFailed">${failed}</span></span>
        </div>
        <p class="muted" id="bulkProgressHint">If you close this page, you can always reopen the job under <strong>Bulk issue → Job history</strong>.</p>
      </div>
    </section>
  `;

  return adminLayout({
    title: 'Bulk issue progress',
    active: 'bulk-issue',
    body,
    notice,
    extraScripts: `<script>window.__BULK_ISSUE_ACTIVE_JOB__ = ${serializeForScript(job && job.id ? job.id : '')};</script>`
  });
}

function renderJobsPage({ syncStatus, bulkJobs = [], notice = '' }) {
  const s = syncStatus || {};
  const processing = (bulkJobs || []).filter((j) => String(j.status || '') === 'processing');
  const body = `
    <section class="admin-header">
      <div>
        <p class="admin-section-label">Background jobs</p>
        <h2>Sync and job status</h2>
        <p>Shows the current GitHub persistence queue and any in-progress bulk issuance runs.</p>
      </div>
    </section>
    ...
    <section class="admin-panel">
      <div class="admin-section-heading">
        <div>
          <p class="admin-section-label">GitHub persistence</p>
          <h3>Data sync queue</h3>
        </div>
      </div>
      <div class="inline-actions" style="margin: 0 0 14px 0;">
        <form method="post" action="/admin/jobs/sync">
          <input type="hidden" name="scope" value="all" />
          <button type="submit" class="button button--ghost">Force sync all data</button>
        </form>
        <form method="post" action="/admin/jobs/sync">
          <input type="hidden" name="scope" value="badges" />
          <button type="submit" class="button button--ghost">Sync badges</button>
        </form>
        <form method="post" action="/admin/jobs/sync">
          <input type="hidden" name="scope" value="templates" />
          <button type="submit" class="button button--ghost">Sync templates</button>
        </form>
        <form method="post" action="/admin/jobs/sync">
          <input type="hidden" name="scope" value="settings" />
          <button type="submit" class="button button--ghost">Sync settings</button>
        </form>
        <form method="post" action="/admin/jobs/sync">
          <input type="hidden" name="scope" value="uploads" />
          <button type="submit" class="button button--ghost">Sync uploads</button>
        </form>
        <form method="post" action="/admin/jobs/sync">
          <input type="hidden" name="scope" value="logs" />
          <button type="submit" class="button button--ghost">Sync logs</button>
        </form>
      </div>
      <div class="stats-grid">
        <article class="stat-card"><span>Enabled</span><strong>${escapeHtml(String(Boolean(s.enabled)))}</strong></article>
        <article class="stat-card"><span>In flight</span><strong>${escapeHtml(String(Boolean(s.inFlight)))}</strong></article>
        <article class="stat-card"><span>Queued</span><strong>${escapeHtml(String(s.queuedCount || 0))}</strong></article>
      </div>
      <div class="panel panel--surface" style="padding: 18px; border-radius: 18px;">
        <p class="muted" style="margin:0 0 8px 0;"><strong>Repo</strong>: <code>${escapeHtml(s.repo || '')}</code></p>
        <p class="muted" style="margin:0 0 8px 0;"><strong>Branch</strong>: <code>${escapeHtml(s.branch || '')}</code></p>
        <p class="muted" style="margin:0 0 8px 0;"><strong>Last reason</strong>: ${escapeHtml(s.lastReason || '')}</p>
        <p class="muted" style="margin:0 0 8px 0;"><strong>Started</strong>: ${escapeHtml(s.startedAt || '')}</p>
        <p class="muted" style="margin:0;"><strong>Finished</strong>: ${escapeHtml(s.finishedAt || '')}</p>
        ${s.lastError ? `<p class="admin-error" style="margin-top:12px;"><strong>Last error</strong>: ${escapeHtml(s.lastError)}</p>` : ''}
      </div>
    </section>
    ...
    <section class="admin-panel">
      <div class="admin-section-heading">
        <div>
          <p class="admin-section-label">Bulk issue</p>
          <h3>Active runs</h3>
        </div>
      </div>
      ${
        processing.length
          ? `<div class="table-wrap">
              <table class="data-table">
                <thead><tr><th>Job</th><th>Status</th><th>Total</th><th>Completed</th><th>Failed</th><th>Progress</th></tr></thead>
                <tbody>
                  ${processing
                    .map((job) => {
                      const pct = typeof job.progressPercent === 'number' ? Math.max(0, Math.min(100, Math.round(job.progressPercent))) : 0;
                      return `<tr>
                        <td><strong>${escapeHtml(job.id || '')}</strong><div class="table-subtext">${escapeHtml(job.badgeTemplateTitle || job.badgeTemplateId || '')}</div></td>
                        <td>${escapeHtml(String(job.status || 'processing'))}</td>
                        <td>${escapeHtml(String(job.totalRows || 0))}</td>
                        <td>${escapeHtml(String(job.completedRows || 0))}</td>
                        <td>${escapeHtml(String(job.failedRows || 0))}</td>
                        <td><a class="text-link" href="/admin/bulk-issue/progress?job=${escapeAttribute(job.id || '')}">${escapeHtml(String(pct))}%</a></td>
                      </tr>`;
                    })
                    .join('')}
                </tbody>
          ...
          : '<p class="muted">No active bulk issue jobs right now.</p>'
      }
    </section>
  `;
  return adminLayout({ title: 'Background jobs', active: 'jobs', body, notice });
}
