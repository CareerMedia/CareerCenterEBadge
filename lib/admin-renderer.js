...
function renderEmailPage({
  ...
  const body = `
    ...
    <section class="admin-panel">
      <form method="post" action="/admin/email/save" class="admin-form admin-form--stacked" id="emailAwardSettingsForm">
        ...
        <p class="muted">Saving with <strong>REST API</strong> checks your key against Brevo (<code>GET /v3/account</code>). If that fails, nothing is saved—fix the key on Render or here, then try again.</p>
      </form>
    </section>
  `;
  ...
  return adminLayout({
    ...
    body,
    notice,
    extraScripts: `<script>window.__EMAIL_BUILTIN__ = ${builtinAward};</script><script>window.__EMAIL_AWARD_STATE__ = ${awardState};</script>`
  });
}
