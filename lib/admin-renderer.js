...
function renderEmailPage({ siteConfig, notice = '', envBrevoKeyConfigured = false }) {
  ...
  const body = `
    ...
    <section class="admin-panel">
      <form method="post" action="/admin/email/save" class="admin-form admin-form--stacked">
        ...
        <p class="muted">In Brevo: <strong>SMTP &amp; API</strong> → <strong>API keys</strong> → create a key with permission to send transactional emails. Add and verify your sender under <strong>Senders</strong> so the address below is allowed.</p>
