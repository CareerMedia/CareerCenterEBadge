...
const { AWARD_EMAIL_TEMPLATE_TAGS, getBuiltinAwardEmailTemplates } = require('./brevo-mailer');
...
function renderEmailPage({
  ...
  const builtins = getBuiltinAwardEmailTemplates();
  const subjectForFormAttr = escapeAttribute(
    String(siteConfig.emailAwardSubjectTemplate || '').trim() || builtins.subject
  );
  const htmlForForm = escapeHtml(
    String(siteConfig.emailAwardHtmlTemplate || '').trim() || builtins.html
  );
  const textForForm = escapeHtml(
    String(siteConfig.emailAwardTextTemplate || '').trim() || builtins.text
  );
  ...
  const body = `
    ...
    <section class="admin-panel">
      <form method="post" action="/admin/email/save" class="admin-form admin-form--stacked">
        ...
        <p class="muted">Use the placeholders below (curly braces, case-insensitive). Empty fields fall back to the built-in default. HTML is sent as-is after placeholders are filled; keep markup simple and trustworthy.</p>
        ...
        <label>
          ...
          <input type="text" name="emailAwardSubjectTemplate" maxlength="998" value="${subjectForFormAttr}" />
        </label>
        <label>
          <span>HTML body</span>
          <textarea name="emailAwardHtmlTemplate" rows="16" class="email-template-textarea">${htmlForForm}</textarea>
        </label>
        <label>
          <span>Plain-text body</span>
          <textarea name="emailAwardTextTemplate" rows="14" class="email-template-textarea">${textForForm}</textarea>
        </label>
  ...
  return adminLayout({ title: 'Email', active: 'email', body, notice });
}
