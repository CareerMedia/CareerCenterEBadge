...
function renderBulkIssuePage({ templates = [], jobs = [], notice = '' }) {
  const templateOptions = templates
    .map((template) => `<option value="${escapeAttribute(template.id)}">${escapeHtml(template.title)}</option>`)
    .join('');
  ...
  const body = `
    <section class="admin-header">
      <div>
        ...
        <p>Upload a CSV with recipient name and email, validate fields, and track job progress while credentials are created.</p>
      </div>
      <div class="inline-actions">
    ...
    <section class="admin-panel">
      <form method="post" action="/admin/bulk-issue/start" class="admin-form admin-form--stacked" id="bulkIssueForm">
        <div class="form-grid form-grid--two">
          <label>
          ...
          <label class="form-span-2">
            ...
            <textarea id="bulkIssueCsv" name="csvContent" rows="10" placeholder="awardee_name,awardee_email,issue_date&#10;Jane Doe,jane@csun.edu,April 28, 2026" required></textarea>
          </label>
        </div>
        <div class="inline-actions">
          <button type="submit">Start bulk issue job</button>
