    ...
    if (row.apiKeySource) {
      const src =
        row.apiKeySource === 'env'
          ? 'REST key from BREVO_API_KEY (host env)'
...
function renderEmailPage({ siteConfig, notice = '', envBrevoKeyConfigured = false, envSmtpConfigured = false }) {
  ...
  const body = `
    ...
    <section class="admin-panel">
      <form method="post" action="/admin/email/save" class="admin-form admin-form--stacked">
        ...
        <p class="muted"><strong>Sending can run without checking the box below</strong> if <code>BREVO_API_KEY</code> is set on the server <em>and</em> a verified sender email is saved here—useful when GitHub blocks storing API keys in JSON. Otherwise, turn on the checkbox and supply credentials for the transport you choose.</p>
        <p class="muted">In Brevo: <strong>SMTP &amp; API</strong> → <strong>API keys</strong> (REST) or <strong>SMTP</strong> tab (SMTP key + relay host). Add and verify your sender under <strong>Senders</strong>.</p>
        ...
        ${
          envBrevoKeyConfigured
            ? '<p class="muted">The host environment variable <code>BREVO_API_KEY</code> is set. It takes priority over a saved key when sending.</p>'
            : ''
        }

        <label class="toggle-check">
          <input type="checkbox" name="emailBrevoClearApiKey" value="true" />
          <span>Remove saved API key from these settings (optional; use only if you will rely on <code>BREVO_API_KEY</code> on the server)</span>
        </label>
        ...
        <p class="muted">Set <strong>Public site URL</strong> on the Settings tab so badge links in emails use a full <code>https://</code> URL.</p>
    ...
    <section class="admin-panel">
      <div class="admin-section-heading">
        <div>
          <p class="admin-section-label">Message template</p>
          <h3>Recipients see</h3>
        </div>
      </div>
      <p><strong>Subject:</strong> Your {Badge-Name} Badge Has Been Awarded</p>
      <pre class="email-template-preview">Hello [Recipient Name],
