  ...
  <body data-page="home">
    ${renderTopBar(0, siteConfig, {
      ...
      actions: [
        ...
        { href: 'lookup/', label: 'Find by email' },
...
function renderGeneratorPage(siteConfig, templates, certificateTemplate, options = {}) {
  ...
  const widgetMode = Boolean(options.widgetMode);
  const modeFolder = widgetMode ? 'widget' : 'generator';
  const relativePath = options.relativePath || (fixedTemplate ? `${modeFolder}/${fixedTemplate.id}/` : `${modeFolder}/`);
  const depth = options.depth == null ? (fixedTemplate ? 2 : 1) : options.depth;
  const canonical = widgetMode ? '' : buildCanonical(siteConfig, relativePath);
  const templatePayload = templates.map((template) => {
    const cert = getCertificateTemplateForTemplate(template, certificateTemplate);
    return {
      ...template,
      badgeImage: relativePathFromDepth(template.badgeImage, depth),
      certificateBackground: relativePathFromDepth((cert.backgroundImage || template.certificateBackground || certificateTemplate.backgroundImage), depth),
      certificateTemplate: {
        ...cert,
        backgroundImage: relativePathFromDepth((cert.backgroundImage || template.certificateBackground || certificateTemplate.backgroundImage), depth)
      }
    };
  });
  const payload = {
    ...
    widgetMode,
    pageKind: widgetMode ? (fixedTemplate ? 'widget-specific' : 'widget-general') : (fixedTemplate ? 'specific' : 'general'),
    certificateTemplate: {
      ...certificateTemplate,
  ...
  const templateOptions = templates.map((template) => `<option value="${escapeAttribute(template.id)}">${escapeHtml(template.title)}</option>`).join('');
  const title = fixedTemplate ? `${fixedTemplate.title} Generator` : 'General Credential Generator';
  const intro = fixedTemplate ? `Create the ${fixedTemplate.title} badge and matching certificate.` : 'Enter the recipient details to create a polished public badge page and matching certificate PDF.';
  const leadBadge = fixedTemplate ? `<span class="pill pill--accent">Locked to ${escapeHtml(fixedTemplate.title)}</span>` : '<span class="pill pill--accent">Any configured badge type</span>';
  const bodyClass = widgetMode ? 'widget-body' : '';
  const htmlClass = widgetMode ? 'widget-html' : '';
  const widgetLayoutClass = widgetMode ? ` generator-layout--widget-${fixedTemplate && fixedTemplate.widgetLayout === 'stacked' ? 'stacked' : 'split'}` : '';
  const wrapperOpen = widgetMode ? '<main class="widget-shell"><section class="widget-card">' : '<main class="page-stack page-stack--generator">';
  const wrapperClose = widgetMode ? '</section></main>' : '</main>';
  const stylesPath = `${'../'.repeat(depth)}assets/public.css`;
  const siteJsPath = `${'../'.repeat(depth)}assets/site.js`;
  const inner = `
      ${widgetMode ? '' : `
      ...
      <section class="${widgetMode ? 'widget-section' : 'page-section'}">
        <div class="${widgetMode ? '' : 'wrap '}generator-layout${widgetMode ? ` generator-layout--widget${widgetLayoutClass}` : ''}">
          <div class="generator-main">
            <article class="panel panel--form panel--surface ${widgetMode ? 'widget-panel' : ''}">
              <div class="panel-heading">
                <div>
                  <p class="section-label">Issue a credential</p>
                  <h2>${widgetMode ? 'Credential widget' : 'Credential generator'}</h2>
                </div>
              </div>
              <form id="publicGeneratorForm" class="certificate-form certificate-form--generator">
                <div class="form-grid form-grid--two">
                  ...
                  <label>
                    ...
                    <input type="email" id="publicGeneratorEmail" name="awardeeEmail" placeholder="recipient@domain.edu" required />
                  </label>
                  <label>
                ...
                <div class="form-actions form-actions--generator form-actions--spaced">
            ...
            <article class="panel panel--surface generator-result ${widgetMode ? 'widget-panel' : ''}" id="generatorResultCard" hidden>
              <div class="panel-heading">
                <div>
              ...
              <div class="generator-success-stack${widgetMode ? ' generator-success-stack--widget' : ''}">
          ...
          <aside class="generator-aside">
            <article class="panel panel--surface panel--preview ${widgetMode ? 'widget-panel' : ''}">
              <div class="panel-heading">
                <div>
              ...
              <div class="generator-profile-grid">
                ...
                <div class="detail-card generator-template-details">
                  <div>
                    ...
                    <p id="generatorTemplateMeaning" class="generator-template-copy">Select a badge template to view the official public summary.</p>
                  </div>
                  <div>
...
<html lang="en"${htmlClass ? ` class="${htmlClass}"` : ''}>
  ...
  <body data-page="generator" class="${bodyClass}">
    ${widgetMode ? '' : renderTopBar(depth, siteConfig, {
...
function renderLookupPage(siteConfig) {
  const canonical = buildCanonical(siteConfig, 'lookup/');
  return `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>${escapeHtml(siteConfig.siteName)} | Find My Badge</title>
    <meta name="description" content="Find your badges by email address. Only matching credentials are shown." />
    ${canonical ? `<link rel="canonical" href="${escapeAttribute(canonical)}" />` : ''}
    <link rel="stylesheet" href="../assets/public.css" />
  </head>
  <body data-page="lookup">
    ${renderTopBar(1, siteConfig, {
      subtitle: 'Recipient lookup',
      actions: []
    })}
    <main class="page-stack">
      <section class="hero-shell hero-shell--registry">
        <div class="wrap hero-shell__inner hero-shell__inner--registry">
          <div class="hero-copy">
            <p class="eyebrow">Public badge recovery</p>
            <h1>Find your badges by email</h1>
            <p class="lede">Enter the email used when the badge was issued. Results only show badges attached to that email.</p>
          </div>
        </div>
      </section>
      <section class="page-section">
        <div class="wrap">
          <div class="panel panel--surface panel--search">
            <form id="emailLookupForm" class="search-bar search-bar--wide">
              <label class="sr-only" for="emailLookupInput">Enter your email</label>
              <input id="emailLookupInput" type="email" placeholder="name@domain.edu" required />
              <button type="submit">Find my badges</button>
            </form>
            <p id="emailLookupStatus" class="muted">Search results are private to the email you enter.</p>
            <div id="emailLookupResults" class="credential-list"></div>
          </div>
        </div>
      </section>
    </main>
...
function renderBadgePage(badge, siteConfig, certificateTemplate) {
  ...
  const effectiveCertificateTemplate = {
    ...(badge.certificateTemplateApplied || certificateTemplate),
    backgroundImage: relativePathFromDepth(((badge.certificateTemplateApplied || {}).backgroundImage || badge.certificateBackground || certificateTemplate.backgroundImage), 2)
  };
  ...
  const sections = normalizeVerificationSections(badge);
  const skills = Array.isArray(badge.skills) ? badge.skills : [];
  const standards = Array.isArray(badge.standards) ? badge.standards : [];
    ...
    primaryCards.push(`
          <article class="panel panel--surface panel--credential-fact">
            ...
            <dl class="detail-list">
              ...
              <div><dt>Issued by</dt><dd>${escapeHtml(trust.issuerName)}</dd></div>
              <div><dt>Organization</dt><dd>${escapeHtml(trust.issuerOrganization || siteConfig.organizationName)}</dd></div>
              <div><dt>Credential ID</dt><dd>${escapeHtml(badge.id)}</dd></div>
              <div><dt>Verification hash</dt><dd><button type="button" class="hash-box" data-copy-text="${escapeAttribute(badge.verificationHash || '')}" title="Click to copy verification hash"><span>${escapeHtml(badge.verificationHash || '')}</span><small>Click to copy</small></button></dd></div>
            </dl>
          </article>`);
  }
  if (sections.meaning) {
    primaryCards.push(`
          <article class="panel panel--surface panel--meaning">
            ...
            <p>${escapeHtml(badge.publicSummary || badge.description || badge.meaning)}</p>
            ${sections.criteria ? `<div class="divider"></div>
            <p class="section-label">Eligibility or criteria</p>
            <p>${escapeHtml(badge.criteria)}</p>` : ''}
          </article>`);
  }
  if (sections.issuerTrust) {
    primaryCards.push(`
          <article class="panel panel--surface panel--verification">
            <p class="section-label">Issuer trust</p>
            <h2>${escapeHtml(trust.issuerTrustLabel || 'Official issuer')}</h2>
            <p>${escapeHtml(trust.issuerVerificationNote || '')}</p>
            <dl class="detail-list detail-list--stacked">
              <div><dt>Issuer website</dt><dd><a href="${escapeAttribute(trust.issuerWebsite)}" target="_blank" rel="noreferrer">${escapeHtml(trust.issuerWebsite)}</a></dd></div>
              <div><dt>Contact email</dt><dd><a href="mailto:${escapeAttribute(trust.issuerContactEmail)}">${escapeHtml(trust.issuerContactEmail)}</a></dd></div>
              <div><dt>Career Center</dt><dd><a href="${escapeAttribute(trust.careerCenterUrl)}" target="_blank" rel="noreferrer">${escapeHtml(trust.careerCenterUrl)}</a></dd></div>
            </dl>
          </article>`);
  }
  ...
  if (sections.evidence) {
    secondaryCards.push(`
          <article class="panel panel--surface">
            <p class="section-label">Evidence and criteria</p>
            <h2>Proof and supporting material</h2>
            <p>${escapeHtml(evidence.description || evidence.prompt || 'This badge can include supporting evidence and proof of work.')}</p>
            <dl class="detail-list detail-list--stacked">
              <div><dt>Evidence label</dt><dd>${escapeHtml(evidence.label)}</dd></div>
              <div><dt>Submitted evidence</dt><dd>${evidence.submissionUrl ? `<a href="${escapeAttribute(evidence.submissionUrl)}" target="_blank" rel="noreferrer">${escapeHtml(evidence.submissionUrl)}</a>` : escapeHtml(evidence.submissionNote || 'No supporting evidence link was supplied for this credential.')}</dd></div>
              <div><dt>Example evidence</dt><dd>${evidence.exampleUrl ? `<a href="${escapeAttribute(evidence.exampleUrl)}" target="_blank" rel="noreferrer">${escapeHtml(evidence.exampleUrl)}</a>` : 'Not configured'}</dd></div>
            </dl>
          </article>`);
  }
  if (sections.skills) {
    secondaryCards.push(`
          <article class="panel panel--surface">
            <p class="section-label">Skills and standards</p>
            <h2>Capabilities recognized</h2>
            <div class="chip-cloud">
              ${(skills.length ? skills : ['No mapped skills yet']).map((skill) => `<span class="chip">${escapeHtml(skill)}</span>`).join('')}
            </div>
            <div class="divider"></div>
            <p class="section-label">Standards or mappings</p>
            <div class="chip-cloud chip-cloud--muted">
              ${(standards.length ? standards : ['No standards mapped']).map((standard) => `<span class="chip chip--muted">${escapeHtml(standard)}</span>`).join('')}
            </div>
          </article>`);
  }
  if (sections.pathway) {
    secondaryCards.push(`
          <article class="panel panel--surface">
            <p class="section-label">Pathway</p>
            <h2>${escapeHtml(pathway.pathwayTitle || 'Standalone credential')}</h2>
            <p>${escapeHtml(pathway.pathwayDescription || 'This badge can also be used as part of a broader stacked pathway.')}</p>
            <div class="pathway-rail">
              ${(pathway.pathwayItems && pathway.pathwayItems.length ? pathway.pathwayItems : [badge.badgeTitle]).map((item, index) => `<span class="pathway-step ${String(item).toLowerCase() === String(badge.badgeTitle).toLowerCase() ? 'is-active' : ''}">${index + 1}. ${escapeHtml(item)}</span>`).join('')}
            </div>
          </article>`);
  }
  const linkedinShare = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(badge.publicUrl)}`;
  const emailShare = `mailto:?subject=${encodeURIComponent(`${badge.badgeTitle} badge from ${trust.issuerName}`)}&body=${encodeURIComponent(`View the credential for ${badge.awardeeName}: ${badge.publicUrl}`)}`;
  const openBadgeJson = 'open-badge.json';
  const vcJson = 'credential.json';
  const detailsJson = 'details.json';
  const emailSignatureHtml = buildEmailSignatureHtml({
    ...badge,
    badgeImage: /^https?:\/\//i.test(badge.badgeImage || '') ? badge.badgeImage : buildCanonical(siteConfig, badge.badgeImage),
    publicUrl: badge.publicUrl
  }, siteConfig);
  const emailSignatureText = buildEmailSignaturePlainText({
    ...badge,
    badgeImage: /^https?:\/\//i.test(badge.badgeImage || '') ? badge.badgeImage : buildCanonical(siteConfig, badge.badgeImage),
    publicUrl: badge.publicUrl
  }, siteConfig);
...
<html lang="en">
  ...
  <body data-page="badge">
    ${renderTopBar(2, siteConfig, { subtitle: 'Official badge verification', compact: true })}
    ...
    <main class="page-stack page-stack--badge">
      <section class="hero-shell hero-shell--badge">
        <div class="wrap badge-hero-grid">
          <div class="hero-copy">
            ...
            <p class="lede">Awarded to <strong>${escapeHtml(badge.awardeeName)}</strong>. This badge is valid, issued by ${escapeHtml(trust.issuerName)}, and marked as never expiring.</p>
            <div class="pill-row">
            ...
            <div class="form-actions badge-hero-actions badge-hero-actions--spaced">
              <button type="button" class="button-secondary" data-copy-url="${escapeAttribute(badge.publicUrl)}">Copy verification link</button>
              <a class="button-secondary button-secondary--link" href="${escapeAttribute(linkedinShare)}" target="_blank" rel="noreferrer">Share on LinkedIn</a>
              <a class="button-secondary button-secondary--link" href="${escapeAttribute(emailShare)}">Share by email</a>
      ...
      ${primaryCards.length ? `<section class="page-section page-section--dense">
        <div class="wrap badge-detail-grid badge-detail-grid--rich">
${primaryCards.join('')}
        </div>
      </section>` : ''}

      ${secondaryCards.length ? `<section class="page-section page-section--dense">
        <div class="wrap badge-detail-grid badge-detail-grid--rich">
${secondaryCards.join('')}
        </div>
      </section>` : ''}
      ...
      <section class="page-section">
        <div class="wrap split-grid split-grid--badge-bottom">
          ${sections.certificate ? `<article class="panel panel--surface panel--certificate">
            <div class="panel-heading">
              <div>
          ...
          <article class="panel panel--surface panel--issuer-statement">
            <p class="section-label">Sharing and metadata</p>
            <h2>Portable credential data</h2>
            <p>Use the official verification page, metadata exports, and trust signals to share this credential across systems and websites.</p>
            <div class="form-actions form-actions--stacked">
              <a class="button-link" href="${escapeAttribute(detailsJson)}" target="_blank" rel="noreferrer">Open details JSON</a>
              <a class="button-secondary button-secondary--link" href="${escapeAttribute(openBadgeJson)}" target="_blank" rel="noreferrer">Open Open Badge metadata</a>
              <a class="button-secondary button-secondary--link" href="${escapeAttribute(vcJson)}" target="_blank" rel="noreferrer">Open VC-style metadata</a>
              <button type="button" class="button-secondary" data-copy-url="${escapeAttribute(badge.publicUrl)}">Copy verification URL</button>
            </div>
            <div class="divider"></div>
            <div class="email-signature-block">
              <div class="email-signature-block__copy">
                <p class="section-label">Email signature</p>
                <h3>Add this badge to your signature</h3>
                <p>Copy a ready-to-paste signature badge for Gmail or Outlook, then paste it directly into your email signature editor.</p>
              </div>
              <div class="email-signature-preview email-signature-preview--surface">
                <a class="email-signature-preview__chip" href="${escapeAttribute(badge.publicUrl)}" target="_blank" rel="noreferrer">
                  <img src="${escapeAttribute(badgeImageForPage)}" alt="${escapeAttribute(badge.badgeTitle)} badge image" class="email-signature-preview__icon" />
                  <span class="email-signature-preview__text">
                    <strong>Certified ${escapeHtml(badge.badgeTitle)}</strong>
                    <small>Links to this verification page</small>
                  </span>
                </a>
              </div>
              <div id="emailSignatureCopySource-${escapeAttribute(badge.id)}" class="email-signature-copy-source" aria-hidden="true">${emailSignatureHtml}</div>
              <div class="email-signature-actions">
                <button type="button" class="button-secondary" data-copy-target="#emailSignatureCopySource-${escapeAttribute(badge.id)}" data-copy-text="${escapeAttribute(emailSignatureText)}">Copy email signature</button>
                <p class="email-signature-help">After copying, paste it into your Gmail or Outlook signature editor.</p>
              </div>
            </div>
          </article>
        </div>
    ...
    <script>window.BADGE_PAGE_DATA = ${serializeForScript({
      ...
      publicUrl: badge.publicUrl,
      fileNameSuffix: effectiveCertificateTemplate.fileNameSuffix,
      openBadgeJsonUrl: openBadgeJson,
      credentialJsonUrl: vcJson
    })};</script>
    <script>window.CERTIFICATE_TEMPLATE = ${serializeForScript(effectiveCertificateTemplate)};</script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>
    <script src="../../assets/site.js"></script>
  </body>
</html>`;
...
function ensureDocsScaffold() {
  ...
  ensureDir(path.join(PATHS.docsDir, 'widget'));
  ensureDir(path.join(PATHS.docsDir, 'lookup'));
}

function resolveBadgeContext(ctx) {
  const siteConfig = (ctx && ctx.siteConfig) || loadSiteConfig();
  const certificateTemplate = (ctx && ctx.certificateTemplate) || loadCertificateTemplate();
  return { siteConfig, certificateTemplate };
}

/**
 * Write the four files for a single badge under docs/badges/<slug>/.
 * Caller may pass a shared { siteConfig, certificateTemplate } when publishing
 * many badges in a row.
 */
function publishBadgeArtifacts(badge, ctx) {
  if (!badge || !badge.slug) {
    return;
  }
  const { siteConfig, certificateTemplate } = resolveBadgeContext(ctx);
  ensureDir(PATHS.docsBadgesDir);
  ...
  writeJson(path.join(badgeDir, 'open-badge.json'), badgeToOpenBadge(badge, siteConfig));
  writeJson(path.join(badgeDir, 'credential.json'), badgeToVcJson(badge, siteConfig));
}

/**
 * Remove the published folder for one badge slug. No-op if absent.
 */
function unpublishBadgeArtifacts(slug) {
  const safeSlug = String(slug || '').trim();
  if (!safeSlug) {
    return;
  }
  const badgeDir = path.join(PATHS.docsBadgesDir, safeSlug);
  if (fs.existsSync(badgeDir)) {
    fs.rmSync(badgeDir, { recursive: true, force: true });
  }
}

/**
 * Rewrite the shared artifacts that depend on the full badge list:
 *   docs/data/badges.json, both badge-links.csv files, docs/sitemap.xml,
 *   and docs/index.html (home page stats).
 * Does NOT touch any per-badge folder, generator/widget pages, registry, lookup,
 * 404, robots, or templates.json.
 */
function publishBadgeIndexes(ctx) {
  ensureDocsScaffold();
  const siteConfig = (ctx && ctx.siteConfig) || loadSiteConfig();
  const templates = (ctx && ctx.templates) || loadBadgeTemplates();
  const badges = sortBadgesDescending((ctx && ctx.badges) || loadBadges());
...
/**
 * Rewrite all template- and site-config-driven artifacts:
 *   generator pages (root + per template), widget pages (root + per template),
 *   docs/data/{templates,site-config,certificate-template}.json,
 *   registry, lookup, 404, robots, and home (templates impact the home cards).
 * Does NOT touch any per-badge folder.
 */
function publishTemplateAssets(ctx) {
  ensureDocsScaffold();
  const siteConfig = (ctx && ctx.siteConfig) || loadSiteConfig();
  const certificateTemplate = (ctx && ctx.certificateTemplate) || loadCertificateTemplate();
  const templates = (ctx && ctx.templates) || loadBadgeTemplates();
  ...
  writeJson(path.join(PATHS.docsDataDir, 'site-config.json'), siteConfigForPublicDocs());
  ...
  const docsWidgetDir = path.join(PATHS.docsDir, 'widget');
  removeDirectoryContents(docsWidgetDir);
  ensureDir(docsWidgetDir);
  writeText(path.join(docsWidgetDir, 'index.html'), renderGeneratorPage(siteConfig, templates, certificateTemplate, {
    widgetMode: true,
    relativePath: 'widget/'
  }));
  for (const template of templates) {
    const widgetDir = path.join(docsWidgetDir, template.id);
    ensureDir(widgetDir);
    writeText(path.join(widgetDir, 'index.html'), renderGeneratorPage(siteConfig, templates, certificateTemplate, {
      widgetMode: true,
      fixedTemplateId: template.id,
      depth: 2,
      relativePath: `widget/${template.id}/`
    }));
  }

  writeText(path.join(PATHS.docsRegistryDir, 'index.html'), renderRegistryPage(siteConfig));
  const docsLookupDir = path.join(PATHS.docsDir, 'lookup');
  ensureDir(docsLookupDir);
  writeText(path.join(docsLookupDir, 'index.html'), renderLookupPage(siteConfig));
...
/**
 * Re-emit per-badge artifacts only for badges that use a given template. Used
 * after a template's badge image / certificate background changes so the
 * affected badge pages reflect the new look without rebuilding every badge.
 */
function publishBadgesForTemplate(templateId, ctx) {
  const id = String(templateId || '').trim();
  if (!id) {
    return 0;
  }
  const { siteConfig, certificateTemplate } = resolveBadgeContext(ctx);
  const badges = (ctx && ctx.badges) || loadBadges();
  let count = 0;
  for (const badge of badges) {
    if (String(badge.badgeTemplateId || '') !== id) {
      continue;
    }
    publishBadgeArtifacts(badge, { siteConfig, certificateTemplate });
    count += 1;
  }
  return count;
}
...
/**
 * Remove the per-template generator and widget folders for a deleted template.
 * The shared generator/widget index pages are rewritten by publishTemplateAssets.
 */
function removeGeneratorAndWidgetForTemplate(templateId) {
  const id = String(templateId || '').trim();
  if (!id) {
    return;
  }
  const generatorDir = path.join(PATHS.docsDir, 'generator', id);
  const widgetDir = path.join(PATHS.docsDir, 'widget', id);
  if (fs.existsSync(generatorDir)) {
    fs.rmSync(generatorDir, { recursive: true, force: true });
  }
