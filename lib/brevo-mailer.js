...
function getBuiltinAwardEmailTemplates() {
  const p = getBuiltinAwardEmailParts();
  return { subject: p.subject, html: p.html, text: p.text };
}

function resolveAwardEmailTemplates(siteConfig, badge) {
  const b = getBuiltinAwardEmailParts();
  const templates = Array.isArray(siteConfig.emailAwardTemplates) ? siteConfig.emailAwardTemplates : [];
  const badgeTplId = String((badge && badge.badgeTemplateId) || '').trim();
  let catalogEntry = null;
  if (badgeTplId) {
    try {
      catalogEntry = loadBadgeTemplates().find((e) => e.id === badgeTplId) || null;
    } catch {
      catalogEntry = null;
    }
  }
  const assignedFromCatalog = String((catalogEntry && catalogEntry.emailAwardTemplateId) || '').trim();
  const defId = String(siteConfig.emailAwardDefaultTemplateId || '').trim();
  const chosenId =
    assignedFromCatalog && templates.some((t) => t.id === assignedFromCatalog)
      ? assignedFromCatalog
      : defId && templates.some((t) => t.id === defId)
        ? defId
        : templates[0]
          ? templates[0].id
          : 'default';
  const emailTpl = templates.find((t) => t.id === chosenId) || templates[0];
  if (!emailTpl) {
    return { subjectTpl: b.subject, htmlTpl: b.html, textTpl: b.text };
  }
  const subjectTpl = String(emailTpl.subject || '').trim() || b.subject;
  let bodyHtml = String(emailTpl.bodyHtml || '').trim();
  let bodyPlain = String(emailTpl.bodyPlain != null ? emailTpl.bodyPlain : '').replace(/\r\n/g, '\n');
  if (!bodyHtml && bodyPlain) {
    bodyHtml = wrapEmailHtmlDocument(plainBodyToHtmlFragment(bodyPlain));
  }
  if (!bodyPlain.trim() && bodyHtml) {
  ...
  return { subjectTpl, htmlTpl: bodyHtml, textTpl: bodyPlain };
}
...
function buildAwardEmailContent(siteConfig, badge, { recipientName, badgeName, badgeUrl, badgeId }) {
  const ctx = buildAwardTokenContext(siteConfig, { recipientName, badgeName, badgeUrl, badgeId });
  const { subjectTpl, htmlTpl, textTpl } = resolveAwardEmailTemplates(siteConfig, badge);
  return {
...
function buildEmailLogEntry(siteConfig, badge, result, errorMessage = '', logExtras = {}) {
  ...
  const toEmail = String((badge && badge.awardeeEmail) || '').trim().toLowerCase();
  const badgeUrl = getAbsoluteBadgeUrl(siteConfig, badge);
  const subject = buildAwardEmailContent(
    siteConfig,
    badge,
    {
